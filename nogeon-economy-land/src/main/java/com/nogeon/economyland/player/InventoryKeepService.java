...
public final class InventoryKeepService {
    ...
    public static void onLivingDeath(LivingDeathEvent event) {
        ...
        if (hasAndConsumeScroll(player)) {
            ...
            backupAndClearCurios(player, persistedData);
    ...
    public static void onPlayerClone(PlayerEvent.Clone event) {
        ...
        if (originalPersisted.getBoolean(KEEP_INVENTORY_TAG)) {
            ...
            restoreCurios(player, originalPersisted);
            ...
            playerPersisted.remove(SAVED_CURIOS_TAG);
    ...
    public static void onPlayerRevived(team.creative.playerrevive.api.event.PlayerRevivedEvent event) {
        ...
        if (persisted.getBoolean(KEEP_INVENTORY_TAG)) {
            ...
            restoreCurios(player, persisted);
    ...
    private static final String SAVED_CURIOS_TAG = "NoGeonKeepCuriosItems";
    ...
    public static void backupAndClearCurios(ServerPlayer player, CompoundTag persistedData) {
        try {
            ...
            if (unwrappedInventory != null) {
                ...
                if (curiosMap instanceof java.util.Map<?, ?> map) {
                    ListTag savedCurios = new ListTag();
                    
                    for (java.util.Map.Entry<?, ?> entry : map.entrySet()) {
                        String slotIdentifier = (String) entry.getKey();
                        Object slotInventory = entry.getValue();
                        
                        Object stacks = slotInventory.getClass().getMethod("getStacks").invoke(slotInventory);
                        if (stacks instanceof net.minecraftforge.items.IItemHandlerModifiable handler) {
                            for (int slot = 0; slot < handler.getSlots(); slot++) {
                                ItemStack stack = handler.getStackInSlot(slot);
                                if (!stack.isEmpty()) {
                                    CompoundTag itemNbt = new CompoundTag();
                                    itemNbt.putString("SlotType", slotIdentifier);
                                    itemNbt.putInt("SlotIndex", slot);
                                    stack.save(itemNbt);
                                    savedCurios.add(itemNbt);
                    ...
                    if (!savedCurios.isEmpty()) {
                        persistedData.put(SAVED_CURIOS_TAG, savedCurios);
                        System.out.println("[InventoryKeepService] Backed up and cleared " + savedCurios.size() + " Curios items for " + player.getName().getString());
                    }
                }
            }
        } catch (Exception e) {
            System.err.println("[InventoryKeepService] Failed to backup Curios items: " + e.getMessage());
        }
    }

    public static void restoreCurios(Player player, CompoundTag persistedData) {
        if (!persistedData.contains(SAVED_CURIOS_TAG, net.minecraft.nbt.Tag.TAG_LIST)) {
            return;
        }
        ListTag savedCurios = persistedData.getList(SAVED_CURIOS_TAG, net.minecraft.nbt.Tag.TAG_COMPOUND);
        try {
            Class<?> curiosApiClass = Class.forName("top.theillusivec4.curios.api.CuriosApi");
            Object helper = curiosApiClass.getMethod("getCuriosHelper").invoke(null);
            Object curiosInventory = helper.getClass()
            ...
            if (unwrappedInventory != null) {
                ...
                if (curiosMap instanceof java.util.Map<?, ?> map) {
                    int restoredCount = 0;
                    for (int i = 0; i < savedCurios.size(); i++) {
                        CompoundTag itemNbt = savedCurios.getCompound(i);
                        String slotType = itemNbt.getString("SlotType");
                        int slotIndex = itemNbt.getInt("SlotIndex");
                        ItemStack stack = ItemStack.of(itemNbt);
                        
                        Object slotInventory = map.get(slotType);
                    ...
                    System.out.println("[InventoryKeepService] Restored " + restoredCount + " Curios items for " + player.getName().getString());
                }
            }
        } catch (Exception e) {
            System.err.println("[InventoryKeepService] Failed to restore Curios items: " + e.getMessage());
        } finally {
            persistedData.remove(SAVED_CURIOS_TAG);
        }
    }
    ...
    public static void onPlayerRespawn(PlayerEvent.PlayerRespawnEvent event) {
        if (event.getEntity() instanceof ServerPlayer player) {
            ...
            try {
                ...
                Object helper = curiosApiClass.getMethod("getCuriosHelper").invoke(null);
                helper.getClass()
                    .getMethod("syncCurios", net.minecraft.world.entity.LivingEntity.class)
                    .invoke(helper, player);
                System.out.println("[InventoryKeepService] Synced Curios for " + player.getName().getString() + " upon respawn.");
            } catch (Exception ignored) {
            }
        }
    }
